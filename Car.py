...
class Car():
    ...
    def __init__(self, vehicle, world):
        ...
        self.append_sensors()
        self.vehicle_list = self.get_vehicle_list(self.world)
    ...
    def get_vehicle_list(self, world):
        '''获得已存在的车辆列表'''
        return world.get_actors().filter('vehicle.*')

    def get_accelerometer(self):
        a_x, a_y, a_z = self.imu_sensor.accelerometer
    ...
    #         waypoint = self.get_waypoint(vehicle)
    #         if waypoint.road_id == self.road_id:
    #             if waypoint.lane_id == self.lane_id:
    ...
    #         if car.vehicle.id == vehicle.id:
    #             return car
    #     return None
    
    def get_waypoint_distance(self, vehicle):
        '''求距离'''
        if vehicle is None:
            return self.waypoint.next_until_lane_end(0.5)/2
        self.waypoint_list = self.get_waypoint_list(self.waypoint, 0.5)
        v_waypoint = self.get_waypoint(vehicle)
        v_length = self.get_waypoint_list(v_waypoint, 0.5)
        return 0.5 * (abs(len(v_length) - len(self.waypoint_list)))
    ...
    def get_back_vehicle(self, road_id, lane_id, length):
        ...
        for i in range(len(self.vehicle_list)):
            if i != self.id:
                i_waypoint = self.get_waypoint(self.vehicle_list[i])
