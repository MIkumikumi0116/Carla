...
class CarlaEnviroment:
    ...
    def Get_Transform(self,x = 0,y = 0,z = 0,p = 0,yaw = 0,r = 0):
        '''获取Carla.Transform'''
        return carla.Transform(carla.Location(x,y,z),carla.Rotation (p,yaw,r))
...
class SpawnCars:
    ...
    def Spawn_car_with_camera(self):
        ...
        while True:
            ...
            car = custom_car.CarWithCarema(car_bp,car_transform,cam_transform)
            if car == NULL:
                continue
            break
    ...
    def Genratar_cars(self):
        ...
        new_car_list = []       #由于不明原因，生成汽车同时启用自动驾驶会使自动驾驶不起作用，因此在new_car_list暂存新生成的汽车，后面再依次启用自动驾驶
    ...
    def Genratar_cars_near_position(self):
        ...
        #选取符合条件的坐标
        coordinates = [i for i in CarlaEnviroment.Get_Spawn_Points() if i.location.distance(spawn_coordinate) <= tolerance]
